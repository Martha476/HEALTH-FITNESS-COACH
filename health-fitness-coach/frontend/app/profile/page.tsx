...
export default function ProfilePage() {
  ...
  useEffect(() => {
    ...
    setIsNewUser(true); setIsEditing(true);
    setDirtyFields(new Set()); setSaveAttempted(false);
    setSaveStatus("idle"); setSaveMessage("");

    const load = async () => {
      try {
        const res = await fetch(`${API_URL}/api/profile/${user.id}`, { headers: getAuthHeaders() });
        if (res.ok) {
          const data = await res.json();
          if (data && Object.keys(data).length > 2) {
            setProfile({ ...EMPTY_PROFILE, name: user.name || "", ...data });
            setIsNewUser(false); setIsEditing(false); return;
          }
        }
      } catch {}
  ...
  useEffect(() => {
    ...
    if (redirectCountdown === 1) { router.push("/dashboard"); return; }
  ...
  const handleSave = async () => {
    if (!isEditing) {
      setIsEditing(true); setDirtyFields(new Set());
      setSaveAttempted(false); setSaveStatus("idle"); setSaveMessage(""); return;
    }
    ...
    setSaveStatus("saving"); setSaveMessage("Saving your profile...");

    // ── FIX: Set startWeight on first save only, never overwrite it ──────────
    ...
    const toSave: Profile = {
      ...
      // On new profile: startWeight = currentWeight. On updates: preserve original.
      startWeight: existingStart > 0 ? existingStart : profile.currentWeight,
    };

    setProfile(toSave);

    if (!user) { setSaveStatus("error"); setSaveMessage("❌ Not logged in."); return; }

    // Save to localStorage — progress page reads "userProfile" key
    ...
    <div className="min-h-[calc(100vh-4rem)] w-full py-8 px-4 sm:px-6 lg:px-8 bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900">
      <div className="w-full max-w-4xl mx-auto">
        ...
        <Section icon="📏" title="Body Measurements" sub="Track your physical metrics">
          {/* ── Unit selector FIRST so labels below update immediately ─────── */}
          <div className="mb-5">
          ...
          {/* Start weight info box — shown after first save */}
