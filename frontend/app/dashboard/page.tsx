...
import { useUnit } from "../hooks/useUnit";  // ✅ shared unit hook

import { FontAwesomeIcon } from "@fortawesome/react-fontawesome";
import {
  faHome, faRobot, faDumbbell, faUtensils, faChartBar,
  faUser, faCog, faQuestionCircle, faSignOutAlt,
  faArrowLeft, faArrowRight, faFire, faStopwatch,
  faGlassWater, faTrophy, faBars,
...
const QUICK_ACTIONS = [
  { href: "/ai-coach",  img: "/images/download.jpg",                                                                                           icon: faRobot,    label: "Chat with AI Coach",    border: "border-green-500 hover:border-green-400"     },
  { href: "/workouts",  img: "/images/Gym Workout_ Overhead Lunges for Strength & Fitness.jpg",                                                icon: faDumbbell, label: "Generate Workout Plan", border: "border-blue-500 hover:border-blue-400"       },
  { href: "/nutrition", img: "/images/Discover simple and natural ways to strengthen your immune system with healthy foods, daily habits,.jpg", icon: faUtensils, label: "Log Today's Meals",     border: "border-emerald-500 hover:border-emerald-400" },
  { href: "/progress",  img: "/images/Morning Mindfulness.jpg",                                                                                icon: faChartBar, label: "Track My Progress",     border: "border-cyan-500 hover:border-cyan-400"       },
];

// ─── Storage helpers ──────────────────────────────────────────────────────────
function scopedKey(userId: string, key: string) { return `user_${userId}_${key}` }
    ...
    <>
      ...
      <aside className={[
        ...
        <div className={`flex items-center gap-3 px-4 py-5 border-b border-slate-700/60 ${collapsed ? "justify-center" : ""}`}>
          <div className="w-9 h-9 rounded-xl bg-green-500/20 border border-green-500/40 flex items-center justify-center text-xl shrink-0">
          ...
          {NAV_ITEMS.map((item) => {
            ...
            return (
              <Link key={item.href} href={item.href} onClick={onClose}
                ...
                <span className="text-base leading-none shrink-0 w-5 flex justify-center"><FontAwesomeIcon icon={item.icon} /></span>
        ...
        <div className="p-3 border-t border-slate-700/60 space-y-2">
          ...
          <button onClick={onToggleCollapse}
            ...
            <span className="text-base w-5 flex justify-center"><FontAwesomeIcon icon={collapsed ? faArrowRight : faArrowLeft} /></span>
          ...
          <button onClick={logout}
            ...
            <span className="text-base w-5 flex justify-center"><FontAwesomeIcon icon={faSignOutAlt} /></span>
  ...
  return (
    <div className="bg-slate-800 rounded-2xl shadow-lg p-5 border border-slate-700 hover:shadow-2xl hover:border-slate-600 transition-all duration-300">
      ...
      <div className="flex items-center justify-between">
        ...
        <span className="text-3xl w-10 flex text-slate-600 justify-center"><FontAwesomeIcon icon={icon} /></span>
      </div>
    </div>
  )
}
...
export default function Dashboard() {
  ...
  // ✅ FIX: use shared unit hook — reacts to profile changes automatically
  const { unit, convert } = useUnit(user?.id)
  ...
  const [stats, setStats] = useState({
    caloriesBurned: 0, workoutCount: 0, totalMinutes: 0, waterIntake: 0,
  })
  ...
  // ✅ FIX: weightInfo stores RAW values in their original unit from profile
  // The unit hook converts them for display
  const [weightInfo, setWeightInfo] = useState<{
    current: number; target: number; rawUnit: "lbs" | "kg";
  } | null>(null)
  ...
  // Only redirect if no user AND no token in localStorage
  useEffect(() => {
  ...
  const loadData = useCallback(() => {
    ...
    if (hasProfile) {
      // ✅ FIX: store raw values + raw unit, display via useUnit hook
      const rawUnit: "lbs" | "kg" = profile.preferredUnit === "metric" ? "kg" : "lbs"
      setWeightInfo({
    ...
    const token = localStorage.getItem("authToken") || localStorage.getItem("fitcoach_token")
    if (token) {
      // Sync nutrition data from backend
      fetch(`/api/meals?userId=${user.id}`, {
        ...
        .then((data) => {
          if (data?.response) {
            console.log("✓ Backend nutrition sync:", data.response)
            // Backend confirms meal sync - UI already updated via localStorage
          }
        })
        .catch(() => {})
  ...
  // ✅ FIX: convert raw weight values to preferred unit for display
  const displayCurrent = weightInfo
    ? convert(weightInfo.current, weightInfo.rawUnit)
    : 0
  const displayTarget = weightInfo
    ? convert(weightInfo.target, weightInfo.rawUnit)
    : 0
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        <header className="h-14 flex items-center gap-3 px-4 border-b border-slate-700/60 bg-slate-900/95 backdrop-blur-sm shrink-0">
          ...
          <div className="flex items-center gap-2 shrink-0">
            <Link href="/ai-coach"
              ...
              <FontAwesomeIcon icon={faRobot} /><span>Ask Coach</span>
            </Link>
        ...
        <main className="flex-1 overflow-y-auto">
          <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6">
            ...
            {/* ✅ FIX: Weight cards — always shows preferred unit */}
            ...
            <section className="grid grid-cols-2 lg:grid-cols-4 gap-4">
              <StatCard icon={faFire}       label="Calories Burned Today" value={stats.caloriesBurned} unit="kcal"    color="text-orange-400" image="/images/Calorie Deficit Calculator – Calculate, Plan & Maintain Your Ideal Weight.jpg" />
              <StatCard icon={faDumbbell}   label="Workouts Today"        value={stats.workoutCount}   unit="sessions" color="text-blue-400"   image="/images/Gym Workout_ Overhead Lunges for Strength & Fitness.jpg" />
              <StatCard icon={faStopwatch}  label="Active Minutes Today"  value={stats.totalMinutes}   unit="min"      color="text-yellow-400" image="/images/Sneaky Exercise Hacks for Maximum Results.jpg" />
              <StatCard icon={faGlassWater} label="Water Intake Today"    value={stats.waterIntake}    unit="glasses"  color="text-cyan-400"   image="/images/Natural Way to Support Your Teeth.jpg" />
            </section>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-400">
                      {new Date(recentWorkout.date).toLocaleDateString("en-US", { month: "short", day: "numeric" })} • {recentWorkout.duration > 0 ? `${recentWorkout.duration} min` : "< 1 min"}
                    </p>
                  </div>
                </div>
            ...
            <section className="bg-slate-800 rounded-2xl shadow-lg border border-slate-700 hover:shadow-2xl transition-shadow duration-300 overflow-hidden">
              ...
              <div className="px-5 pb-5 space-y-3">
                ...
                <div className="flex gap-2 mt-2">
                  <Link href="/nutrition" className="flex-1 bg-emerald-600 hover:bg-emerald-700 text-white py-2.5 rounded-xl font-semibold transition-colors block text-center text-sm">
                    🥗 Log a Meal
                  </Link>
                  <Link href="/nutrition" className="flex-1 bg-slate-700 hover:bg-slate-600 text-slate-200 py-2.5 rounded-xl font-semibold transition-colors block text-center text-sm border border-slate-600">
                    View Full Plan
                  </Link>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                {QUICK_ACTIONS.map((action) => (
                  <Link key={action.href} href={action.href}
                    ...
                    <div className="text-3xl mb-2 text-slate-500"><FontAwesomeIcon icon={action.icon as any} /></div>
